...
/**
 * Methods supporting a list of usernote records.
 *
 * @since  1.6
 */
class UserNotesModelUserNotes extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__usernotes') . ' AS a');
		...
		if ($assoc)
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_usernotes.item'))
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_usernotes.usernote')
				);
		}
