...
class JFormFieldGmkbValue extends JFormField
{
	...
	protected function getInput()
	{
		...
		$html[] = '<input type="checkbox" id="'.$this->id.'_dchk" onclick="gmkb_doDefault(this)" '.($this->value ? '' : 'checked ').'style="vertical-align:initial" />';
		...
		$html[] = '<select id="' . $this->id . '_gmkb" onchange="gmkb_doGmkb(this)" style="width:5em">';
		...
		if (!$scripted) {
			$scripted = true;
			$script = array();
			$script[] = 'function gmkb_doDefault (elm) {';
			...
			$script[] = '	}';
			$script[] = '}';
			$script[] = 'function gmkb_doGmkb (elm) {';
